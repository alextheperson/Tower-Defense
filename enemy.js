...
class Enemy {
  constructor (tiles, restOfPath, side) {
    ...
    for (let i = 0; i < this.path.length; i++) {
      ...
      let nextSide = (currentTile.tower) ? "tower" : sides[[Math.sign(this.path[i][0] - this.path[i + 1][0]), Math.sign(this.path[i][1] - this.path[i + 1][1])].join(",")]
  ...
  update() {
    if (this.pointLerpAmount >= 1) {
      if (this.pathProgress < this.coordinatePath.length - 2) {
        ...
        this.tiles.enemies.splice(this.tiles.enemies.indexOf(this), 1)
        health -= 1 * this.damage
        // alert("your tower lost damage")
      }
    } else {
